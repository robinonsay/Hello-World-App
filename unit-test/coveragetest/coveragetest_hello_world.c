...
/*
** File: coveragetest_hello_world.c
**
** Purpose:
...
#include "hello_world_coveragetest_common.h"
#include "ut_hello_world.h"
...
void Test_HELLO_WORLD_Main(void)
{
    ...
    /*
     * HELLO_WORLD_Main does not return a value,
     ...
     * The first is for HELLO_WORLD_Init().  As this is in the same
     ...
     * the HELLO_WORLD_Init() failure path was taken.
     */
    HELLO_WORLD_Main();
     ...
     */
    UtAssert_True(HELLO_WORLD_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "HELLO_WORLD_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)HELLO_WORLD_Data.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_PIPE_ERR_EID, "SAMPLE APP: SB Pipe Read Error, App Will Exit");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "HELLO_WORLD_PIPE_ERR_EID generated (%u)",
...
void Test_HELLO_WORLD_Init(void)
{
    ...
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Init(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 5, "CFE_ES_WriteToSysLog() called");
}

void Test_HELLO_WORLD_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void HELLO_WORLD_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        HELLO_WORLD_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_INVALID_MSGID_ERR_EID, "SAMPLE: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(HELLO_WORLD_CMD_MID);
    FcnCode   = HELLO_WORLD_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    HELLO_WORLD_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(HELLO_WORLD_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    HELLO_WORLD_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "HELLO_WORLD_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_HELLO_WORLD_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void HELLO_WORLD_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        HELLO_WORLD_NoopCmd_t          Noop;
        HELLO_WORLD_ResetCountersCmd_t Reset;
        HELLO_WORLD_ProcessCmd_t       Process;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_COMMANDNOP_INF_EID, NULL);

    HELLO_WORLD_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_True(EventTest.MatchCount == 1, "SAMPLE_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = HELLO_WORLD_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_COMMANDRST_INF_EID, NULL);
    ...
    /* note this will end up calling HELLO_WORLD_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = HELLO_WORLD_PROCESS_CC;
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_COMMAND_ERR_EID, "Invalid ground command code: CC = %d");
    HELLO_WORLD_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_HELLO_WORLD_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void HELLO_WORLD_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(HELLO_WORLD_SEND_HK_MID);

    /* Set message id to return so HELLO_WORLD_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    HELLO_WORLD_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &HELLO_WORLD_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &HELLO_WORLD_Data.HkTlm.TlmHeader.Msg,
...
void Test_HELLO_WORLD_NoopCmd(void)
{
    /*
     * Test Case For:
     * void HELLO_WORLD_NoopCmd( const HELLO_WORLD_Noop_t *Msg )
     */
    HELLO_WORLD_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;
    ...
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "HELLO_WORLD_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_HELLO_WORLD_ResetCounters(void)
{
    /*
     * Test Case For:
     * void HELLO_WORLD_ResetCounters( const HELLO_WORLD_ResetCounters_t *Msg )
     */
    HELLO_WORLD_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_COMMANDRST_INF_EID, "SAMPLE: RESET command");

    UT_TEST_FUNCTION_RC(HELLO_WORLD_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "HELLO_WORLD_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_HELLO_WORLD_ProcessCC(void)
{
    /*
     * Test Case For:
     * void  HELLO_WORLD_ProcessCC( const HELLO_WORLD_Process_t *Msg )
     */
    HELLO_WORLD_ProcessCmd_t TestMsg;
    HELLO_WORLD_Table_t      TestTblData;
    void *                  TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the HELLO_WORLD_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Process(&TestMsg), CFE_SUCCESS);
     ...
     */
    UT_SetDefaultReturnValue(UT_KEY(CFE_TBL_GetAddress), CFE_TBL_ERR_UNREGISTERED);
    UT_TEST_FUNCTION_RC(HELLO_WORLD_Process(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
}

void Test_HELLO_WORLD_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool HELLO_WORLD_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, HELLO_WORLD_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    HELLO_WORLD_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "HELLO_WORLD_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    HELLO_WORLD_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "HELLO_WORLD_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_HELLO_WORLD_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(HELLO_WORLD_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return HELLO_WORLD_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + HELLO_WORLD_TBL_ELEMENT_1_MAX;
    UT_TEST_FUNCTION_RC(HELLO_WORLD_TblValidationFunc(&TestTblData), HELLO_WORLD_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_HELLO_WORLD_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(HELLO_WORLD_Main);
    ADD_TEST(HELLO_WORLD_Init);
    ADD_TEST(HELLO_WORLD_ProcessCommandPacket);
    ADD_TEST(HELLO_WORLD_ProcessGroundCommand);
    ADD_TEST(HELLO_WORLD_ReportHousekeeping);
    ADD_TEST(HELLO_WORLD_NoopCmd);
    ADD_TEST(HELLO_WORLD_ResetCounters);
    ADD_TEST(HELLO_WORLD_ProcessCC);
    ADD_TEST(HELLO_WORLD_VerifyCmdLength);
    ADD_TEST(HELLO_WORLD_TblValidationFunc);
    ADD_TEST(HELLO_WORLD_GetCrc);
}
